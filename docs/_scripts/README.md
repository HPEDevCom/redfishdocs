...
- `_resmap.sh`: adapts the `ilo6_resmap.md` file to the Redocly MD format.
  Moreover, compared to iLO 5, URIs labeled \_Collection of\_ point to the collection URI,
  and not to the members URI. As an example, in iLO 5,
  the \_Collection of Chassis\_ points to `(#Chassis)`. Redocly's  iLO 6 `resmap.md`,points to `(#ChassisCollection)`.
